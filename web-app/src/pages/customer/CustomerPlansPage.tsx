...
import { Zap, CheckCircle2, CreditCard, ShieldCheck, Building2 } from 'lucide-react';
        ...
        {plansLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {activePlans.map((plan: any, i: number) => (
              ...
              >
                <Card className="h-full flex flex-col justify-between hover:border-aura-primary/50 transition-all">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="font-bold text-aura-text text-lg">{plan.name || plan.title}</h3>
                      <Badge variant="info">{plan.durationDays || plan.duration || 30} Days</Badge>
                    </div>
